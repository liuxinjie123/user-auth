...
public class TokenUtil {
     ...
     * 加密token.
     */
    public String getToken(String userId, String userMobile) {
    ...
    public Map<String, String> parseToken(String token) {
        try {
            ...
            Claim userMobile = decodedjwt.getClaim("userMobile");
            Claim timeStamp = decodedjwt.getClaim("timeStamp");
            map.put("userId", userId.asString());
            map.put("userMobile", userMobile.asString());
            map.put("timeStamp", timeStamp.asLong().toString());
            return map;
        } catch (JWTVerificationException e) {
            throw new TokenVerificationException("token非法");
        }
    }
}
