 ...
 * Redis的核心配置类
 *
 * 这个类提供了两个方法（其实提供了两个bean，这两个bean会加载到spring的context里边，供使用者进行调用)
 * @Configuration 这个标签，通常与@bean结合使用，
 * 当@bean使用到该类的方法上，代表将该方法做为一个bean交给了spring的context
 *
...
public class RedisConfig {
    ...
    public RedisTemplate<String,Object> redisTemplate(RedisConnectionFactory factory){
        ...
        // 4.2 value序列化，因为我们的value大多是通过对象转化过来的，所以使用jackson2JsonRedisSerializer
        redisTemplate.setValueSerializer(jackson2JsonRedisSerializer);
        // 4.3 value序列化，hashmap的序列话
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory){
        ...
        Jackson2JsonRedisSerializer jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer(Object.class);
        ObjectMapper objectMapper = new ObjectMapper();
        /** 2.1 设置objectMapper的访问权限  **/
        objectMapper.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
        /** 2.2 指定序列化输入类型,就是将数据库里的数据按照一定类型存储到redis缓存中。 **/
        /** 最近升级SpringBoot，发现enableDefaultTyping方法过期过期了。可以使用下面的方法代替  **/
        //objectMapper.enableDefaultTyping(ObjectMapper.DefaultTyping.NON_FINAL);
        objectMapper.activateDefaultTyping(LaissezFaireSubTypeValidator.instance, ObjectMapper.DefaultTyping.NON_FINAL, JsonTypeInfo.As.WRAPPER_ARRAY);
        jackson2JsonRedisSerializer.setObjectMapper(objectMapper);
